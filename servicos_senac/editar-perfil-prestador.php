    ...
    try {
        ...
        if (isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_extension, $allowed_extensions)) {
                $new_filename = 'prestador_' . $user_id . '_' . time() . '.' . $file_extension;
                $upload_path = $upload_dir . $new_filename;
                
                if (move_uploaded_file($_FILES['foto_perfil']['tmp_name'], $upload_path)) {
                    $stmt = $pdo->prepare("UPDATE profissionais SET foto_perfil_url = ? WHERE id_usuario = ?");
...
<head>
    ...
    <style>
        ...
        .status-ocupado { background: #f39c12; }
        .status-ausente { background: #e67e22; }
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group form-group-full">
                    ...
                    <div class="photo-upload">
                        <div class="current-photo">
                            <?php if (!empty($prestador['foto_perfil_url']) && file_exists($prestador['foto_perfil_url'])): ?>
                                <img src="<?= htmlspecialchars($prestador['foto_perfil_url']) ?>" alt="Foto atual">
                            <?php else: ?>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="nome" name="nome" value="<?= htmlspecialchars($prestador['nome']) ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="email">Email *</label>
                        <input type="email" id="email" name="email" value="<?= htmlspecialchars($prestador['email']) ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="telefone">Telefone</label>
                        <input type="tel" id="telefone" name="telefone" value="<?= htmlspecialchars($prestador['telefone']) ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="whatsapp">WhatsApp</label>
                        <input type="tel" id="whatsapp" name="whatsapp" value="<?= htmlspecialchars($prestador['whatsapp']) ?>">
                    </div>
                </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="cep" name="cep" value="<?= htmlspecialchars($prestador['cep']) ?>" maxlength="9">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="cidade_id" name="cidade_id">
                            ...
                            <?php foreach ($cidades as $cidade): ?>
                                <option value="<?= $cidade['id_cidade'] ?>" <?= $cidade['id_cidade'] == $prestador['cidade_id'] ? 'selected' : '' ?>>
                ...
                <div class="form-group form-group-full">
                    ...
                    <input type="text" id="endereco_completo" name="endereco_completo" value="<?= htmlspecialchars($prestador['endereco_completo']) ?>" placeholder="Rua, número, bairro">
                </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="area_atuacao" name="area_atuacao" value="<?= htmlspecialchars($prestador['area_atuacao']) ?>" placeholder="Ex: Eletricista, Encanador, etc.">
                    </div>
                    
                    <div class="form-group">
                        <label for="experiencia_anos">Anos de Experiência</label>
                        <input type="number" id="experiencia_anos" name="experiencia_anos" value="<?= isset($prestador['experiencia_anos']) ? htmlspecialchars($prestador['experiencia_anos']) : '' ?>" min="0" max="50">
                    </div>
                </div>
                
                <div class="form-group form-group-full">
                    <label for="descricao_perfil">Descrição do Perfil</label>
                    <textarea id="descricao_perfil" name="descricao_perfil" placeholder="Conte um pouco sobre sua experiência, especialidades e diferenciais..."><?= htmlspecialchars($prestador['descricao_perfil']) ?></textarea>
                </div>
                ...
                <div class="form-group form-group-full">
                    ...
                    <div class="status-options">
                        <div class="status-option">
                            <input type="radio" id="disponivel" name="disponibilidade" value="disponivel" <?= $prestador['disponibilidade'] === 'disponivel' ? 'checked' : '' ?>>
                        ...
                        <div class="status-option">
                            <input type="radio" id="ocupado" name="disponibilidade" value="ocupado" <?= $prestador['disponibilidade'] === 'ocupado' ? 'checked' : '' ?>>
                            <label for="ocupado">
                        ...
                        <div class="status-option">
                            <input type="radio" id="ausente" name="disponibilidade" value="ausente" <?= $prestador['disponibilidade'] === 'ausente' ? 'checked' : '' ?>>
                            <label for="ausente">
                                <div class="status-indicator status-ausente"></div>
                                Ausente
                            </label>
                        </div>
                        <div class="status-option">
                            <input type="radio" id="offline" name="disponibilidade" value="offline" <?= $prestador['disponibilidade'] === 'offline' ? 'checked' : '' ?>>
                            <label for="offline">
                ...
                <div class="form-actions">
                    ...
                    <a href="dashboard-prestador.php" class="btn btn-secondary">
